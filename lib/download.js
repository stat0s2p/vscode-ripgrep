...
async function getAssetFromGithubApi(opts, assetName, downloadFolder) {
  ...
  let fileServerHost = process.env.RIPGREP_FILE_HOST || "https://github.com";

  let url = `${fileServerHost}/microsoft/ripgrep-prebuilt/releases/download/${opts.version}/${assetName}`;

  console.log(`Downloading from ${url}`);
  console.log(`Downloading to ${assetDownloadPath}`);

  downloadOpts.headers.accept = "application/octet-stream";
  await download(url, assetDownloadPath, downloadOpts);
}
